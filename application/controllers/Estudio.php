...
//        }
    ...
    function agregar() {
        ...
        if ($this->form_validation->run('estudio') == false) {
            ...
            if ($this->estudio_model->add('estudio', $data) == TRUE) {
                
                $id_estudio = $this->db->insert_id();
                $acciones = array(
                    'usuario' => $this->session->userdata('id'),
                    'accion_id' => 1,
                    'accion' => 'Agrega el estudio : '.$this->input->post('titulo').' '.$this->input->post('tipo'),
                    'modulo' => 2,
                    'fecha_registro' => date('Y-m-d h:i:s')
                );
                
                if ($this->consola_model->add('consola',$acciones) == TRUE){
                    $this->session->set_flashdata('success', 'estudio agregados con exito!');
           ...
           //si existen archivos
            if (isset($_FILES)){
              if ($this->adjuntar_archivo($id_estudio,"estudio") ==TRUE){
