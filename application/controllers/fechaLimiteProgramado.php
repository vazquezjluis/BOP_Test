...
class FechaLimiteProgramado extends CI_Controller {
...
//        }
    ...
    function agregar(){
        ...
        $this->fechalimiteprogramado_model->edit('fechalimiteprogramado',array ('estado'=>0),' 1 ',' 1 ');
        
            $data = array(
                    'fecha'   => $this->input->post('fecha_limite'),
                    'f_registro'        => date('Y-m-d'),
                    'usuario'           => $this->session->userdata('id'),
                    'estado' => 1
            );
            if ($this->fechalimiteprogramado_model->add('fechalimiteprogramado',$data) == TRUE)
            {   
                $acciones = array(
                    'usuario' => $this->session->userdata('id'),
                    'accion_id' => 1,
                    'accion' => 'Agrega la nueva fecha limite para programar un menu: '.$this->db->insert_id().' -_- fecha : '.$this->input->post('fecha_limite'),
                    'modulo' => 1,
                    'fecha_registro' => date('Y-m-d')
                );
                if ($this->consola_model->add('consola',$acciones) == TRUE){

                    $this->session->set_flashdata('success','Fecha registrada con éxito!');
