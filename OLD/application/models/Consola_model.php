...
class Consola_model extends CI_Model {
    ...
    function get($table,$fields,$where='',$perpage=0,$start=0,$one=false,$array='array'){
        ...
        $this->db->select('consola.*, usuarios.nombre as usuario');
        $this->db->from($table);
        $this->db->join('usuarios', 'usuarios.idUsuarios= consola.usuario', 'left');
        $this->db->order_by('idConsola','desc');
    ...
    function getById($id){
        $this->db->where('idConsola',$id);
        $this->db->limit(1);
        return $this->db->get('consola')->row();
    }
