<?php
class BingoOasis_model extends CI_Model {
    ...
    public function modificarClave($newClave,$oldClave,$id){
        ...
        if(password_verify($oldClave, $usuario->clave)){
            $this->db->set('clave', password_hash($newClave,PASSWORD_DEFAULT,array('cost'=>12)));
            $this->db->where('idUsuarios',$id);
...
//
//    }
    function buscar_maquina($buscar){
         $data = array();
         // buscando clientes
         $this->db->like('nro_egm',$buscar);
         $this->db->limit(5);
         $data = $this->db->get('maquinas')->result();
...
//
    ...
    public function check_credentials($usr) {
